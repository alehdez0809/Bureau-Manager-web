...
function EditarCuotas() {
            ...
            .then(response => {
              ...
              Promise.all(fetchCuotas)
                .then(results => {
                  ...
                  const edificiosConCuotasFiltrados = edificiosConCuotas.filter(edificio => edificio.tieneCuota);
    ...
    useEffect(() => {
        if (selectedEdificio) {
            axios.get(`http://localhost:4000/api/obtenerCuota/${selectedEdificio}`)
                .then(response => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:4000/api/actualizarCuotas', formulario);
    ...
    return (
        <div className='div-contenedor div-espaciado' style={{marginTop: "150px"}}>
            ...
            <form className='fromInquilino' onSubmit={handleSubmit}>
                <p style={{textAlign: 'justify'}}><b>NOTA: </b>Solo se mostrán en las opciones los edificios 
                que ya tengan cuotas asignadas.</p>
                <div className='container-cuotas'>
                    <div className='column-cuotas'>
                        ...
                        <div className='item-cuotas'>
                            <label htmlFor="cuota_base" className='labelInput'>Ingrese la nueva cuota base (o sin estacionamiento):</label>
                            <br/>
                            <input type='number' id='cuota_base' min={0} max={10000} placeholder='$' required onChange={handleInputChange} value={formulario.cuota_base}/>
                        </div>
                    </div>
                    <div className='column-cuotas'>
                        <div className='item-cuotas'>
