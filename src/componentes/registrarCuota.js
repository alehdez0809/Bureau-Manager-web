...
function RegistrarCuotas() {
          ...
          axios.post(`http://localhost:4000/api/getEdificiosbyCondominio`, { id_condominio: selectedCondominio })
            .then(response => {
              ...
              const fetchCuotas = edificios.map(edificio => 
                axios.get(`http://localhost:4000/api/obtenerCuota/${edificio.id_edificio}`)
              );
              ...
              Promise.all(fetchCuotas)
                .then(results => {
                  ...
                  const edificiosSinCuotas = edificiosConCuotas.filter(edificio => !edificio.tieneCuota);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:4000/api/registrarCuotas', formulario);
    ...
    return (
        <div className='div-contenedor div-espaciado' style={{marginTop: "150px"}}>
            ...
            <form className='fromInquilino' onSubmit={handleSubmit}>
                <p style={{textAlign: 'justify'}}><b>NOTA: </b>Si ya se han registrado y asignado cuotas a 
                    cierto edificio, este ya no 
                    se mostrará entre las opciones.</p>
                <div className='container-cuotas'>
                    <div className='column-cuotas'>
                        <div className='item-cuotas'>
                            <label className='labelInput' htmlFor="condominio">Seleccione un condominio:</label>
                            <br/>
                        ...
                        <div className='item-cuotas'>
                            <label htmlFor="cuota_base" className='labelInput'>Ingrese la cuota base (o sin estacionamiento):</label>
                            <br/>
                            <input type='number' id='cuota_base' min={0} max={10000} placeholder='$' required onChange={handleInputChange}/>
                        </div>
                    </div>
                    <div className='column-cuotas'>
                        <div className='item-cuotas'>
                            <label className='labelInput' htmlFor="condominio">Seleccione un edificio:</label>
                            <br/>
                        ...
                        <div className='item-cuotas'>
                            <label htmlFor="cuota_extra" className='labelInput'>Ingrese la cuota por estacionamiento (opcional):</label>
                            <br/>
                            <input type='number' id='cuota_extra' min={0} max={10000} placeholder='$' onChange={handleInputChange}/>
                        </div>
                    </div>
                </div>
                ...
                <div style={{display: 'flex', justifyContent: 'flex-start'}}>
                    <button className="mi-boton2" type='submit'>Registrar cuotas</button>
                </div>
